...
def read_make_list(filename = 'setup.info'):
	...
	result = parse(lines)
	return result['wrapper'][0], result['headers'], result['sources']
...
def make():
	...
	extensions = [Extension(wrapper,sources = [wrapper+ext]+sourcefiles, 
				include_dirs = headdirs)]
				#include_dirs = headdirs,extra_compile_args = ['/EHsc'])]
	if USE_CYTHON: extensions = cythonize(extensions, annotate = True)
