...
#from libs.modular_core.libmath import pwm_square as pwm_square
...
cdef class TrueValidator:

	#def validate(self):
	cpdef int validate(self):
		#return True
		return 1

#@cython.profile(False)
cdef class ReactionValidator:

	cdef int [:] cnt_dexes
	cdef double [:] counts
	cdef int [:] stochios

	#cdef bint validate(self):
	cpdef int validate(self):
		#can this be a list comprehension / does that help?
		cdef int dex
		for dex in range(len(self.cnt_dexes)):
			if self.counts[self.cnt_dexes[dex]] < self.stochios[dex]:
				return 0

		return 1

	cpdef init(self, int[:] cnt_dexes, 
			double[:] counts, int[:] stochios):
...
cdef class Reaction:
	...
	def __init__(self, double[:] counts, reagents, products):
		re_dexes = np.array([agent[1] for agent in reagents])
		re_stoch = np.array([agent[0] for agent in reagents])
		pr_dexes = np.array([agent[1] for agent in products])
		pr_stoch = np.array([agent[0] for agent in products])
		self.re_dexes = re_dexes
		self.re_stoch = re_stoch
		self.pr_dexes = pr_dexes
		self.pr_stoch = pr_stoch
		self.counts = counts
		self.re_count = len(self.re_dexes)
		self.pr_count = len(self.pr_dexes)

	cpdef react(self):
		cdef int dex
		for dex in range(self.re_count):
			self.counts[self.re_dexes[dex]] -= self.re_stoch[dex]

		for dex in range(self.pr_count):
			self.counts[self.pr_dexes[dex]] += self.pr_stoch[dex]

cdef class Propensity(np.object):

	cpdef double propense(self):
		print 'base propensity class cant propense!'

cdef class PropensitySimple(Propensity):
	...
	def __init__(self, double rate, double[:] counts, int[:] cnt_dexes):
		self.rate = rate
		self.counts = counts
		self.cnt_dexes = cnt_dexes
	...
	cpdef double propense(self):
		...
		cdef int cnt_dex
		for cnt_dex in self.cnt_dexes: population *= self.counts[cnt_dex]
		propensity = population * self.rate
		if propensity > self.propensity_minimum: return propensity
		else: return 0.0

cdef class PropensityVerySimple(Propensity):
#could validators be specific to propensity classes??
	...
	def __init__(self, double rate, double[:] counts, int[:] cnt_dexes):
		self.rate = rate
		self.counts = counts
		self.cnt_dex = cnt_dexes[0]

		self.propensity_minimum = 1e-30

	#cdef void propensity_very_simple(self):
	cpdef double propense(self):
		propensity = self.counts[self.cnt_dex] * self.rate
		if propensity > self.propensity_minimum: return propensity
		else: return 0.0
...
def generate_reaction(count_targets, counts, functions, func_names, 
	...
	cnt_dexes = np.array(cnt_dexes)
	stochios = np.array(stochios)
	...
	if not rate_flag:
		if not has_reagents:
			...
			#valid = validator
			valid = ReactionValidator()
			valid.init(cnt_dexes, counts, stochios)
			if only_one_reagent and single_stochios:
				#correct_propensity = propensity_very_simple
				correct_propensity = PropensityVerySimple(rate, counts, cnt_dexes)

			#elif single_stochios: correct_propensity = propensity_simple
			elif single_stochios: correct_propensity = PropensitySimple(rate, counts, cnt_dexes)
	...
	return correct_propensity, valid, Reaction(counts, reagents, products), reagents, products
...
cdef class PropensityWrapper:

	cdef double [:] propensity_table
	cdef int [:] valid_table
	cdef int [:, :] propensity_lookups

	cdef Propensity [:] propensities

	def __init__(self, propensity_table, valid_table, propensities, 
				validators, reagents, products, rate_call_group):

		self.propensity_table = propensity_table
		self.valid_table = valid_table
		print propensities
		#self.propensities = np.array(propensities, dtype = np.object_)
		self.propensities[:] = np.array(propensities, dtype = Propensity)
		...
		self.propensity_lookups = np.array([uniq(lookup) 
					for lookup in propensity_lookups])
		#self.wraps = [self.generate_wrap(lookup) for 
		#				lookup in propensity_lookups]

	cpdef propensity_wrap(self, int lookup_dex):
		#cdef int dex
		cdef int sub_dex
		lookup = self.propensity_lookups[lookup_dex]
		#for dex in range(len(lookup)):
		for sub_dex in lookup:
			#sub_dex = lookup[dex]
			self.propensity_table[sub_dex] = self.propensities[sub_dex].propense()
			self.valid_table[sub_dex] = self.validators[sub_dex].validate()
...
def wrap_propensities_validators(propensity_table, valid_table, 
		propensities, validators, reagents, products, rate_call_group):
	...
	def generate_wrap(lookup):
		...
		def propensity_wrap():
			...
			for dex in range(leng):
				sub_dex = lookup[dex]
				propensity_table[sub_dex] = propensities[sub_dex].propense()
				valid_table[sub_dex] = validators[sub_dex].validate()
...
def generate_functions(functions, count_targets, counts):
	...
	pwms = [fu.count('pwm_square') > 0 for fu in encoded]
	pwm_flag = False
	for dex in [dex for dex in range(len(pwms)) if pwms[dex]]:
		if not pwm_flag:
			pwm_flag = True
			pwm_square = pwm_sq(encoded[dex], counts, count_targets)

		#encoded[dex] = lm.set_pwm(encoded[dex], counts, count_targets)
		#print encoded[dex]
		encoded[dex] = pwm_square.get_encoded(encoded[dex])
...
cpdef simulate(system_string = '<species>Substrate:10000,Enzyme:5000,ES_Complex:0,Product:0<variables>k:0.5<functions>ratio=Substrate/(Enzyme+k)<reactions>(1)Substrate+(1)Enzyme->1.0->(1)ES_Complex,(1)ES_Complex->0.01->(1)Substrate+(1)Enzyme,(1)ES_Complex->800.0->(1)Enzyme+(1)Product<end>iteration>=10000<capture>increment:time:0.00002<targets>ES_Complex,Product,iteration,time,fixed_time||'):
	...
	counts = np.concatenate((counts, np.array([func(counts) 
	...
	valid_table = np.zeros(propensity_count, dtype = int)
	for dex in range(len(validators)):
		valid_table[dex] = validators[dex].validate()

	for dex in range(len(propensities)):
		propensity_table[dex] = propensities[dex].propense()
	...
	propensities = PropensityWrapper(propensity_table, valid_table, 
		propensities, validators, reagents, products, rate_call_group)
	#propensities = wrap_propensities_validators(
	#	propensity_table, valid_table, 
	#	propensities, validators, reagents, products, rate_call_group)
	...
	while not end_test():
		...
		for prop_dex in range(propensity_count):
			#if validators[prop_dex].validate():
			if valid_table[prop_dex]:
		...
		#reactions[rxn_dex]()
		reactions[rxn_dex].react()
		#propensities[rxn_dex]()
		propensities.propensity_wrap(rxn_dex)
