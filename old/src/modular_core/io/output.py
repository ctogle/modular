...
class output_plan(lfu.plan):
    ...
    def __init__(self,*args,**kwargs):
        ...
        defoutputpath = lset.get_setting('default_output_path')
        if defoutputpath is None:defoutputpath = lfu.get_output_path()
        self._default('save_directory',defoutputpath,**kwargs)
        self._default('save_filename','',**kwargs)
        self._default('targeted',[],**kwargs)
