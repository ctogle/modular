...
###############################################################################
### routine_abstract is the super class for all fitting routines
###############################################################################
...
class routine_abstract(lfu.mobject):
    ...
    # determine if this routine should terminate
    def _finished(self):
        oi = self.iteration + 1 >= self.max_iteration
        ot = time.time() - self.starttime >= self.max_runtime
        ou = self.undos > self.max_undos
        ol = (self.iteration - self.best_capture) > self.max_last_best
        done = oi or ot or ou or ol
        if done:print 'oi,',oi,',ot,',ot,',ou,',ou,',ol,',ol
        return done

    # record the current pspace position
    ...
    # using information from _iterate, apply feedback
    def _feedback(self,information,ran,pspace):
        undo = not self._accept_step(information,ran)
        self._pspace_move(pspace,undo)

    # factor to scale pspace motions per _iterate call
    def _movement_factor(self):
        return 1.0

    # bias movement if undo is False
    # scale movement by factor
    ...
    # capture data based on the current routine state
    ...
    # move to pspace location of best fit
    def _move_to_best(self):
        best_location = self.psp_trajectory[self.best_capture]
        self.parameter_space._move_to(best_location)
