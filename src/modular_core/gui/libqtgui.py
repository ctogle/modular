...
class plot_page(lfu.modular_object_qt):
	...
	def redraw_plot(self, xlab = None, ylab = None, title = None):
		...
		data = self.data
		'''
		if self.parent.propagate_selections:
			...
			self.xdomain = self.parent.xdomain
			self.ydomain = self.parent.ydomain
			self.zdomain = self.parent.zdomain
			self.active_targs = self.parent.active_targs
			self.rewidget(True)
		'''
		data.xdomain = self.parent.xdomain
		data.ydomain = self.parent.ydomain
		data.zdomain = self.parent.zdomain
		data.active_targs = self.parent.active_targs
		qplot.plot(data, xlab, ylab, title, ptype = self.plot_type)
	...
	#	doms = lfu.grab_mobj_by_name(self.xdomain, self.data.data)
	#	cods = [d for d in self.data.data if d.label in self.active_targs]
	#	data = lfu.data_container(domains = doms, codomains = cods)
	#	return data
...
class plot_window(lfu.modular_object_qt):
	...
	def __init__(self, *args, **kwargs):
		...
		#self.impose_default('propagate_selections', True, **kwargs)
	...
	def roll_pages(self):
		...
		while rdex < max_rdex:
			sele.currentIndexChanged.emit(rdex)
			sele.repaint()
